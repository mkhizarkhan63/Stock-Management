  ...
  <body>
    ...
    <div class="container" id="invoice">
      <div class="row">
       ...
       $sql = "SELECT * FROM `sales` INNER JOIN accounts ON accounts.id = sales.acc_id INNER JOIN credit ON credit.sales_id = sales.id WHERE accounts.id = '$id'  AND sales.date between '$s' and '$e' order by sales.date ASC ";
        //echo( $sql);
        ...
        if ($result->num_rows > 0) {
        	...
        	while($row = $result->fetch_assoc()){
        		$total += $row['credit'];
        		?>
        		<tr><td><?= $row['date']?></td><td><?= $row['weight']?></td><td><?= $row['crates_quan']?></td><td><?= $row['rate']?></td><td><?= $row['details']?></td><td><?=  $row['credit']?></td></tr>
